...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://www.bilibili.com/video/BV1Tm8QeZE4a?p=102)


# 绪论
## 1.1 研究背景
当前社会各行业领域竞争压力非常大，随着当前时代的信息化，科学化发展，让社会各行业领域都争相使用新的信息技术，对行业内的各种相关数据进行科学化，规范化管理。这样的大环境让那些止步不前，不接受信息改革带来的信息技术的企业随时面临被淘汰，被取代的风险。所以当今，各个行业领域，不管是传统的教育行业，餐饮行业，还是旅游行业，医疗行业等领域都将使用新的信息技术进行信息革命，改变传统的纸质化，需要人手工处理工作事务的办公环境。软件信息技术能够覆盖社会各行业领域是时代的发展要求，各种数据以及文件真正实现电子化是信息社会发展的不可逆转的必然趋势。本学术团队管理系统也是紧跟科学技术的发展，运用当今一流的软件技术实现软件系统的开发，让家具销售库存管理信息完全通过管理系统实现科学化，规范化，程序化管理。从而帮助信息使用者节省事务处理的时间，降低数据处理的错误率，对于基础数据的管理水平可以起到促进作用，也从一定程度上对随意的业务管理工作进行了避免，同时，学术团队管理系统的数据库里面存储的各种动态信息，也为上层管理人员作出重大决策提供了大量的事实依据。总之，学术团队管理系统是一款可以真正提升使用者的使用效率的软件系统。
## 1.2 目的和意义
信息数据的处理完全依赖人工进行操作，会耗费大量的人工成本，特别是面对大量的数据信息时，传统人工操作不仅不能对数据的出错率进行保证，还容易出现各种信息资源的低利用率与低安全性问题。更有甚者，耽误大量的宝贵时间，尤其是对信息的更新，归纳与统计更是耗财耗力的过程。所以电子化信息管理的出现就能缓解以及改变传统人工方式面临的处境，一方面可以确保信息数据在短时间被高效处理，还能节省人力成本，另一方面可以确保信息数据的安全性，可靠性，并可以实现信息数据的快速检索与修改操作，这些优点是之前的旧操作模式无法比拟的。因此学术团队管理系统为数据信息的管理模式的升级与改革提供了重要的窗口。
## 1.3 论文结构安排
...
# 4 系统设计
学术团队管理系统的设计方案比如功能框架的设计，比如数据库的设计的好坏也就决定了该系统在开发层面是否高效，以及在系统维护层面是否容易维护和升级，因为在系统实现阶段是需要考虑用户的所有需求，要是在设计阶段没有经过全方位考虑，那么系统实现的部分也就无从下手，所以系统设计部分也是至关重要的一个环节，只有根据用户需求进行细致全面的考虑，才有希望开发出功能健全稳定的程序软件。
## 4.1 系统概要设计
...
|3|huiyizhuti|String|会议主题|是|
|4|huiyileixing|String|会议类型|是|
|5|fengmian|String|封面|是|
|6|kaishishijian|datetime|开始时间|是|
|7|jieshushijian|datetime|结束时间|是|
|8|canhuirenyuan|String|参会人员|是|
...
|1|Id|Int|id|否|
|2|addtime|Date|创建时间|是|
|3|zhoubaomingcheng|String|周报名称|是|
|4|zhoubaotupian|String|周报图片|是|
|5|zhoubaoneirong|String|周报内容|是|
|6|faburiqi|date|发布日期|是|
|7|xuehao|String|学号|是|
|8|xingming|String|姓名|是|
|9|thumbsupnum|Integer|赞|是|
|10|crazilynum|Integer|踩|是|
4收藏表
...
|2|addtime|Date|创建时间|是|
|3|huiyizhuti|String|会议主题|是|
|4|kaishishijian|String|开始时间|是|
|5|jieshushijian|String|结束时间|是|
...
|11|fasongshijian|datetime|发送时间|是|
10学生签退表
...
|4|xingming|String|姓名|是|
|5|banji|String|班级|是|
|6|qiantuishijian|datetime|签退时间|是|
|7|beizhu|String|备注|是|
|8|longitude|float|经度|是|
|9|latitude|float|纬度|是|
|10|fulladdress|String|地址|是|
11学术周报评论表
...
|4|xingming|String|姓名|是|
|5|banji|String|班级|是|
|6|qiandaoshijian|datetime|签到时间|是|
|7|beizhu|String|备注|是|
|8|longitude|float|经度|是|
|9|latitude|float|纬度|是|
|10|fulladdress|String|地址|是|
...
## 5.1 学生管理
如图5.1显示的就是学生管理页面，此页面提供给管理员的功能有：对学生信息进行查询，添加，删除以及批量删除操作。

![](/md/blog.012.png)

图5.1 学生管理页面
## 5.2 导师管理
如图5.2显示的就是导师管理页面，管理员可以对导师信息进行添加，修改，删除，查询操作。

![](/md/blog.013.png)

图5.2 导师管理页面
## 5.3 会议类型管理
如图5.3显示的就是会议类型管理页面，管理员可以对会议类型信息信息修改，删除，批量删除以及查询操作。

![](/md/blog.014.png)

图5.3会议类型管理页面
## 5.4 会议通知管理
如图5.4显示的就是会议通知页面，管理员可以对会议通知信息进行添加，修改，删除，查询操作。

![](/md/blog.015.png)

图5.4 会议通知页面
## 5.5 学术周报管理
如图5.4显示的就是学术周报页面，学生和管理员可以对学术周报信息进行添加，修改，删除，查询操作。

![](/md/blog.016.png)

图5.5 学术周报管理页面
## 5.6 学生签到管理
如图5.4显示的就是学生签到页面，学生和管理员可以对学生签到信息进行添加，修改，删除，查询操作。
...
## 5.7 学生签退管理
如图5.4显示的就是学生签退页面，学生和管理员可以对学生签退信息进行添加，修改，删除，查询操作。

![](/md/blog.018.png)

图5.7 学生签退管理页面
## 5.8 确认收到管理
如图5.4显示的就是确认收到页面，管理员可以对教师提交的确认收到信息进行查询，查看和删除。
